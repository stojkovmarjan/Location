...
namespace LocationApi.Controllers
{
    ...
    public class LocationController : ControllerBase
    {
        ...
        {
            return await UploadImage(file, "profile_pics");
        }

        [HttpPost("UploadSignature")]
        public async Task<IActionResult> UploadSignature(IFormFile file)
        {
            return await UploadImage(file, "signatures");
        }


        [NonAction]
        private async Task<IActionResult> UploadImage(IFormFile file, string subDirectory)
        {
            ...
            var uploadDirectory = Path.Combine(Directory.GetCurrentDirectory(), "uploads", subDirectory);
            var success = await _fileService.SaveUploadedFileAsync(file, uploadDirectory);

            if (success)
            {
                return Ok($"File {file.FileName} uploaded successfully to {subDirectory}.");
            }
            else
            {
